...
from pymammotion.mammotion.devices import Mammotion, MammotionBaseCloudDevice
...
async def exponential_backoff(
    func, max_retries: int = 3, initial_delay: float = 1.0, max_delay: float = 10.0, jitter: bool = True
) -> any | None:
...
async def run() -> CloudIOTGateway:
    ...
    try:
        ...
        await exponential_backoff(lambda: cloud_client.get_region(country_code), max_retries=3, initial_delay=5.0)
        await asyncio.sleep(0.5)  # Add delay between calls

        await exponential_backoff(lambda: cloud_client.connect(), max_retries=3, initial_delay=5.0)
        await asyncio.sleep(0.5)

        await exponential_backoff(lambda: cloud_client.login_by_oauth(country_code), max_retries=3, initial_delay=5.0)

        # Remove unused responses check since we're now doing sequential calls

        # Execute remaining calls with retry logic and delays
        await exponential_backoff(lambda: cloud_client.aep_handle(), max_retries=3, initial_delay=5.0)
        await asyncio.sleep(0.5)

        await exponential_backoff(lambda: cloud_client.session_by_auth_code(), max_retries=3, initial_delay=5.0)
        await asyncio.sleep(0.5)

        await exponential_backoff(lambda: mammotion_http.get_all_error_codes(), max_retries=3, initial_delay=5.0)
        ...
        _LOGGER.error(f"Test failed: {e!s}")
        raise
    ...
    _mammotion_mqtt = MammotionCloud(
        AliyunMQTT(
    ...
    try:
        ...
        _LOGGER.error(f"MQTT connection failed: {e!s}")
        raise
    ...
    for device in cloud_client.devices_by_account_response.data.data:
        if device.device_name.startswith("Luba-"):
            dev = MammotionBaseCloudDevice(
                mqtt=_mammotion_mqtt, cloud_device=device, state_manager=MowerStateManager(MowingDevice())
            )
            _devices_list.append(dev)
    # Wrap device commands in error handling
    try:
        await exponential_backoff(
            lambda: _devices_list[0].queue_command("send_todev_ble_sync", sync_type=3), max_retries=3, initial_delay=5.0
        )
        await asyncio.sleep(1)

        await exponential_backoff(
            lambda: _devices_list[0].queue_command("get_report_cfg_stop"), max_retries=3, initial_delay=5.0
        )
        await asyncio.sleep(1)

        await exponential_backoff(
            lambda: _devices_list[0].queue_command("get_report_cfg"), max_retries=3, initial_delay=5.0
        )
        await asyncio.sleep(1)
        ...
        _LOGGER.error(f"Invalid JSON response: {e!s}")
        raise
    except Exception as e:
        _LOGGER.error(f"Device command failed: {e!s}")
        raise
    ...
    try:
        ...
        _LOGGER.error(f"Invalid JSON response in device command: {e!s}")
        raise
    except Exception as e:
        _LOGGER.error(f"Device command execution failed: {e!s}")
        raise
