...
async def test_happy_path_mqtt_command_round_trip() -> None:
    ...
    mqtt_transport = make_mock_transport(TransportType.CLOUD_ALIYUN, connected=True)
    handle = make_mock_handle(mqtt_transport=mqtt_transport)
...
async def test_ble_preferred_when_connected() -> None:
    ...
    mqtt_transport = make_mock_transport(TransportType.CLOUD_ALIYUN, connected=True)
    ble_transport = make_mock_transport(TransportType.BLE, connected=True)
...
async def test_ble_used_when_prefer_ble_set() -> None:
    ...
    handle = make_mock_handle("dev-ble", "BLE-Preferred", prefer_ble=True)
