...
from pymammotion.utility.constant.device_constant import (
    ...
    device_connection,
    PosType,
    WorkMode as PyMammotionWorkMode,
)
...
PYMAMMOTION_COMMAND_FAILURE_EXCEPTIONS = (
    ...
    DeviceOfflineException,
    GatewayTimeoutException,
    NoConnectionException,  # from pymammotion.aliyun.cloud_gateway
...
def get_enum_value(enum_class: Type[Any], value_str: str) -> Optional[int]:
    for member_name in enum_class.__members__:
        ...
        if member_name.replace("_", "").lower() == value_str.lower():
            return enum_class[member_name].value
    logger.warning(
        f"Could not parse '{value_str}' into a valid {enum_class.__name__} member. Parameter will not be set or will use library default."
    )
    return None
...
class StatusFormatter:
    ...
    def format_summary_text(summary: Dict[str, Any]) -> str:
        ...
        display_name = (
            f"{device_name_str} ({nickname_str})" if nickname_str and nickname_str != "N/A" else device_name_str
        )
        ...
        if last_update_ts:
            try:
                ...
                if age_delta.total_seconds() < 1:
                    status_age_str = "<1s ago"
                elif age_delta.total_seconds() < 60:
                    status_age_str = f"{int(age_delta.total_seconds())}s ago"
                elif age_delta.total_seconds() < 3600:
                    status_age_str = f"{int(age_delta.total_seconds() / 60)}m ago"
                else:
                    status_age_str = f"{int(age_delta.total_seconds() / 3600)}h ago"
            except (ValueError, TypeError):
                status_age_str = "InvalidTS"
        ...
        if summary.get("is_charging"):
            ...
            if charge_rate is not None:
                charge_details_parts.append(f"Rate: {charge_rate:.1f}%/m")
            if t_80 is not None and t_80 > 0:
                charge_details_parts.append(f"->80% {int(t_80)}m")
            if t_100 is not None and t_100 > 0:
                charge_details_parts.append(f"->100% {int(t_100)}m")
        ...
        rtk_details_parts = []
        if rtk_stars is not None:
            rtk_details_parts.append(f"{rtk_stars}*")
        if l1_sats is not None:
            rtk_details_parts.append(f"L1:{l1_sats}")
        if l2_sats is not None:
            rtk_details_parts.append(f"L2:{l2_sats}")
        ...
        rssi_val = None
        rssi_unit = "dBm"
        if conn_type == "WiFi" and summary.get("wifi_rssi_dbm") is not None:
            rssi_val = summary.get("wifi_rssi_dbm")
        elif conn_type == "Mobile" and summary.get("mobile_net_rssi_dbm") is not None:
            rssi_val = summary.get("mobile_net_rssi_dbm")
        elif conn_type == "BLE" and summary.get("ble_rssi_dbm") is not None:
            rssi_val = summary.get("ble_rssi_dbm")

        conn_details = f"{conn_type}"
        if rssi_val is not None:
            conn_details += f" ({rssi_val}{rssi_unit})"
        if False:
            lines.append(StatusFormatter._format_field("Connect", conn_details))
        ...
        is_working_type_activity = sys_status_code in [
            PyMammotionWorkMode.MODE_WORKING,
            PyMammotionWorkMode.MODE_PAUSE,
            PyMammotionWorkMode.MODE_RETURNING,
        ]
        ...
        if summary.get("is_working") or summary.get("activity", "").lower() == "paused":
            ...
            if prog_percent is not None:
                prog_line_parts.append(f"Prog: {prog_percent}%")
            if rem_time is not None and rem_time >= 0:
                prog_line_parts.append(f"Rem: {int(rem_time)}m")
            if prog_line_parts:
                lines.append("         " + "  ".join(prog_line_parts))

            work_val_parts = []
            if blade_h is not None:
                work_val_parts.append(f"Blade: {blade_h}mm")
            if speed_mps is not None:
                work_val_parts.append(f"Speed: {speed_mps:.1f}m/s")
            if work_val_parts:
                lines.append(f"{'WorkVals'.ljust(label_w)}: {', '.join(work_val_parts)}")
    ...
    def format_detailed_text(summary: Dict[str, Any]) -> str:
        ...
        preferred_order = [
            ...
            "charge_time_to_80_min",
            "charge_time_to_100_min",
            # Work Progress
            "is_working",
            "work_progress_percent",
            "remaining_job_time_min",
            "total_job_time_min",
            "elapsed_job_time_min",
            "total_area_to_mow_sqm",
            "blade_height_mm",
            "mowing_speed_mps",
            # Location & Area
        ...
        for key, value in summary.items():
            ...
            if key in [
                ...
                if isinstance(value, float):
                    display_items[key] = f"{value:.2f} %/min"
                else:
                    continue
            elif key == "sys_time_stamp":
                try:
                    display_items[key] = datetime.fromtimestamp(int(value)).isoformat()
                except:
...
class MowerController:
    ...
    async def __aenter__(self):
        ...
        if not await self.connect_and_initialize_mower():
            ...
            logger.info(
                f"Successfully connected to: {self.active_mower_device._cloud_device.deviceName if self.active_mower_device else 'Unknown'}"
            )
    ...
    async def _app_shutdown_logic(self):
        ...
        if self.active_mower_device:
            ...
            try:
                ...
                if hasattr(self.active_mower_device, "stop_sync") and callable(
                    getattr(self.active_mower_device, "stop_sync")
                ):
        ...
        if (
            self._mammotion_http
            and hasattr(self._mammotion_http, "close_session")
            and callable(self._mammotion_http.close_session)
        ):
    ...
    async def _login_to_mammotion(self) -> bool:
        ...
        if (
            ...
            or not hasattr(self._mammotion_http, "login_info")
            or not self._mammotion_http.login_info
        ):
    ...
    async def _select_luba_device(self) -> Optional[AliyunDevice]:
        if not (
            hasattr(self._cloud_client, "devices_by_account_response")
            and self._cloud_client.devices_by_account_response
            and self._cloud_client.devices_by_account_response.data
            and self._cloud_client.devices_by_account_response.data.data
        ):
        ...
        luba_device_objects = [
            d for d in all_aliyun_devices if d.deviceName.startswith(("Luba", "Yuka"))
        ]  # Include Yuka
        ...
        if len(luba_device_objects) == 1:
            ...
            logger.info(
                f"Found device: {device_info_selected.deviceName} (Nickname: {device_info_selected.nickName or 'N/A'}, IoT ID: {device_info_selected.iotId})"
            )
        else:
            ...
            while True:
                try:
                    ...
                    if 0 <= choice_idx < len(luba_device_objects):
                        ...
                        logger.info(
                            f"Selected device: {device_info_selected.deviceName} (Nickname: {device_info_selected.nickName or 'N/A'})"
                        )
                        break
                    else:
    ...
    async def _initialize_mqtt_client(self) -> bool:
        if not self._cloud_client:
            raise MowerConnectionError("Cloud client not initialized for MQTT setup.")
        ...
        for (attr_path, obj), err_msg in required_attrs.items():
            ...
            try:
                ...
                if not current:
                    raise AttributeError(f"{attr_path} resolved to a falsy value.")
        ...
        self._mqtt_client = MammotionMQTT(
            ...
            device_name=mqtt_device_name_for_mqtt_auth,
    ...
    async def _initialize_active_mower_device(self, device_info_selected: AliyunDevice):
        ...
        self.active_mower_device = MammotionBaseCloudDevice(
            mqtt=self._cloud_mqtt_wrapper, cloud_device=device_info_selected, state_manager=StateManager(MowingDevice())
        )
    ...
    async def _request_continuous_status_updates(self, reason: str = "Command Executed"):
        ...
        try:
            ...
            await self._execute_mower_command(
                "request_iot_sys",
                params={
                    "rpt_act": RptAct.RPT_START.value,
                    "rpt_info_type": rpt_type_values,
                    "timeout": 10000,
                    "period": 3000,
            ...
            logger.error(
                f"Unexpected error requesting continuous status updates ({reason}): {e_req_status}", exc_info=True
            )

    async def _perform_initial_device_sync(self):
        if not self.active_mower_device:
            raise MowerConnectionError("Active mower device not set for initial sync.")
        ...
        await self._execute_mower_command(
            "allpowerfull_rw", params={"rw_id": 5, "rw": 1, "context": 1}, user_friendly_name="sync_rtk_dock_location"
        )
    ...
    async def _load_mow_defaults(self):
        default_settings = {
            ...
            "blade_height": 60,
            "mowing_laps": "ONE",
            "job_mode": "BORDER_FIRST",
            "obstacle_laps": "NONE",
            "channel_mode": "SINGLE_GRID",
            "ultra_wave": "NORMAL",
            "channel_width": 25,
            "toward": 0,
            "toward_included_angle": 90,
            "toward_mode": "RELATIVE_ANGLE",
            "rain_tactics": 1,
        }
        try:
    ...
    async def _execute_mower_command(
        self, pymammotion_cmd: str, params: Optional[Dict] = None, user_friendly_name: Optional[str] = None
    ):
        ...
        try:
            ...
            logger.debug(
                f"Command '{log_cmd_name}' (library cmd: '{pymammotion_cmd}') executed successfully by library. Response snippet: {'N/A' if result is None else result_str}"
            )
            return result
            ...
            MowerCommandError
        ) as e_mower:  # This case should ideally not happen if _execute itself is the one raising it
            err_msg = f"Command '{log_cmd_name}' (library cmd: '{pymammotion_cmd}') failed: {e_mower}"
            logger.error(err_msg)
            raise
        except Exception as e_unexpected:
            err_msg = (
                f"Unexpected error during '{log_cmd_name}' (library cmd: '{pymammotion_cmd}') execution: {e_unexpected}"
            )
    ...
    async def attempt_to_populate_maps(self, reason: str = "Data Population", force_refresh: bool = False):
        ...
        try:
            ...
            await self._execute_mower_command(
                "get_area_name_list", params={"device_id": device_iot_id}, user_friendly_name=f"{reason}_get_area_names"
            )
            await asyncio.sleep(1.5)
            logger.info(f"({reason}) Attempting to read plans with 'read_plan' (sub_cmd=2)...")
            await self._execute_mower_command(
                "read_plan", params={"sub_cmd": 2, "plan_index": 0}, user_friendly_name=f"{reason}_read_plans"
            )
            ...
            AttributeError
        ) as e_attr:  # This might happen if library changes and a command is no longer on MammotionBaseCloudDevice
            logger.warning(
                f"A command in '{reason}' sequence might not be directly callable on active_mower_device: {e_attr}"
            )
    ...
    def _populate_device_static_info(self, summary_dict: Dict[str, Any]):
        ...
        summary_dict["nickname"] = getattr(
            self.active_mower_device._cloud_device, "nickName", None
        )  # Keep as None if not set
    ...
    def _populate_device_dynamic_info(self, summary_dict: Dict[str, Any], mower_data: MowingDevice):
        if hasattr(mower_data, "report_data") and mower_data.report_data and mower_data.report_data.dev:
            ...
            summary_dict["base_activity_str"] = (
                device_mode(rd_dev.sys_status) if rd_dev.sys_status is not None else "Unknown Status"
            )
    ...
    def _populate_rtk_info(self, summary_dict: Dict[str, Any], mower_data: MowingDevice):
        if hasattr(mower_data, "report_data") and mower_data.report_data and mower_data.report_data.rtk:
            ...
            summary_dict["rtk_status"] = (
                str(RTKStatus.from_value(rd_rtk.status).name) if rd_rtk.status is not None else "N/A"
            )
    ...
    def _populate_work_info(self, summary_dict: Dict[str, Any], mower_data: MowingDevice):
        if hasattr(mower_data, "report_data") and mower_data.report_data and mower_data.report_data.work:
            ...
            if hasattr(rd_work, "progress") and rd_work.progress is not None:
                ...
                remaining_time_min = (
                    rd_work.progress >> 16
                )  # This is actually remaining, not elapsed from this field alone
                summary_dict["total_job_time_min"] = (
                    total_time_min if total_time_min != 0 else None
                )  # Can be 0 if not in job
                summary_dict["remaining_job_time_min"] = (
                    remaining_time_min if remaining_time_min != 0 and total_time_min != 0 else None
                )
                if (
                    total_time_min is not None
                    and remaining_time_min is not None
                    and total_time_min >= remaining_time_min
                ):
            ...
            summary_dict["mowing_speed_mps"] = (
                rd_work.man_run_speed / 100.0
                if hasattr(rd_work, "man_run_speed") and rd_work.man_run_speed is not None
                else None
            )
        else:
    ...
    def _update_charge_rate_estimate(self, current_battery_percent: Optional[int], charge_state_code: Optional[int]):
        ...
        if charge_state_code == 1 and current_battery_percent is not None:  # Charging
            if self._last_battery_reading_percent is not None and self._last_battery_reading_time is not None:
                if current_battery_percent > self._last_battery_reading_percent:  # Ensure battery actually increased
                    ...
                    if time_diff_seconds > 10 and percent_diff > 0:  # Min 10s interval and actual increase
                        ...
                        if self._charge_rate_ewma_percent_per_minute is None:
                            ...
                            self._charge_rate_ewma_percent_per_minute = (current_rate_ppm * self._EWMA_ALPHA) + (
                                self._charge_rate_ewma_percent_per_minute * (1 - self._EWMA_ALPHA)
                            )
                        logger.debug(
                            f"Charge rate updated (EWMA): {self._charge_rate_ewma_percent_per_minute:.2f} %/min (sample: {current_rate_ppm:.2f} %/min over {time_diff_seconds:.1f}s)"
                        )
                ...
                self._last_battery_reading_percent is None or self._last_battery_reading_time is None
            ...
            if (
                self._last_battery_reading_percent is not None or self._last_battery_reading_time is not None
            ):  # Log reset only if there was a baseline
    ...
    def _populate_charging_info(self, summary_dict: Dict[str, Any], mower_data: MowingDevice):
        ...
        if (
            not is_docked_val
            and mower_data.location
            and mower_data.location.RTK
            and hasattr(mower_data.location.RTK, "on_charger")
            and mower_data.location.RTK.on_charger == 1
        ):
            is_docked_val = True
        ...
        if (
            is_charging_val
            and current_battery_val is not None
            and self._charge_rate_ewma_percent_per_minute is not None
            and self._charge_rate_ewma_percent_per_minute > Constants.MIN_CHARGE_RATE_FOR_ESTIMATE
        ):
            ...
            if not (is_charging_val and current_battery_val is not None):
                ...
                self._charge_rate_ewma_percent_per_minute is not None
                and self._charge_rate_ewma_percent_per_minute <= Constants.MIN_CHARGE_RATE_FOR_ESTIMATE
            ):
                logger.debug(
                    f"Not calculating charge times: EWMA rate {self._charge_rate_ewma_percent_per_minute:.2f} <= min_rate {Constants.MIN_CHARGE_RATE_FOR_ESTIMATE}"
                )
    ...
    def _populate_location_info(self, summary_dict: Dict[str, Any], mower_data: MowingDevice):
        if mower_data.location:
            ...
            summary_dict["position_certainty"] = (
                str(PosType(mower_data.location.position_type).name)
                if summary_dict["position_certainty_code"] is not None
                else "N/A"
            )
    ...
    def _determine_activity_and_area(self, summary_dict: Dict[str, Any], mower_data: MowingDevice):
        ...
        if is_docked:
            ...
            if work_zone_hash is not None and work_zone_hash != 0:  # In a numbered zone
                if (
                    hasattr(mower_data, "map")
                    and mower_data.map
                    and hasattr(mower_data.map, "area_name")
                    and mower_data.map.area_name
                ):
                    found_area_obj = next(
                        (
                            area_obj
                            for area_obj in mower_data.map.area_name
                            if hasattr(area_obj, "hash") and area_obj.hash == work_zone_hash
                        ),
                        None,
                    )
    ...
    async def periodic_status_monitor(self):
        ...
        while not self.cli_exit_event.is_set():
            ...
            should_poll = False
            reason_for_poll = []
            if not self.continuous_reporting_started:
                reason_for_poll.append("ContRptOff")
                should_poll = True
            if is_actively_working:
                reason_for_poll.append("Working")
                should_poll = True
            if is_undocked and not is_actively_working:
                reason_for_poll.append("UndockedNotWorking")
                should_poll = True
            if is_stale_or_no_timestamp:
                reason_for_poll.append("StaleData")
                should_poll = True
            ...
            if should_poll:
                logger.debug(
                    f"Polling (Reason: {', '.join(reason_for_poll)}): Requesting full update (get_report_cfg)..."
                )
                ...
                try:
                    ...
                    if new_timestamp_after_poll is not None and (
                        previous_timestamp_before_poll is None
                        or new_timestamp_after_poll > previous_timestamp_before_poll
                    ):
                        polled_and_got_new_data = True
                        logger.debug(f"Poll successful, new data timestamp: {new_timestamp_after_poll}")
                    elif (
                        new_timestamp_after_poll is not None
                        and previous_timestamp_before_poll is not None
                        and new_timestamp_after_poll == previous_timestamp_before_poll
                    ):
                        logger.info(
                            "Poll executed, but server did not provide newer status data (timestamp unchanged)."
                        )
                    else:
                        logger.info(
                            "Poll executed, but new status data seems incomplete or timestamp is missing/unchanged."
                        )
            ...
            if current_data_timestamp is not None:
                if (
                    self._last_printed_sys_time_stamp is None
                    or current_data_timestamp > self._last_printed_sys_time_stamp
                ):
                    should_print_this_cycle = True
                elif (
                    should_poll and polled_and_got_new_data
                ):  # Force print if we explicitly polled and got new data, even if TS didn't change (unlikely but possible)
                    should_print_this_cycle = True
            ...
            if self.last_activity_for_notification and current_activity_for_notification:
                ...
                is_idle_or_completed = any(
                    s in act_low for s in ["idle", "charging", "completed", "standby", "docked"]
                ) or (summary_to_print.get("sys_status_code") == PyMammotionWorkMode.MODE_READY)
                ...
                if isinstance(self.last_progress_for_notification, (int, float)):
                    ...
                    isinstance(self.last_progress_for_notification, str) and "%" in self.last_progress_for_notification
                ):
                    try:
                        progress_val_prev = int(self.last_progress_for_notification.replace("%", ""))
                    except ValueError:
                        pass

                if was_working and is_idle_or_completed and progress_val_prev is not None and progress_val_prev >= 95:
                    logger.info(
                        f"NOTIFICATION: Task likely completed! Prev State: {self.last_activity_for_notification} ({self.last_progress_for_notification}%), Current State: {current_activity_for_notification}"
                    )
    ...
    async def handle_status_command(self, args: List[str]):
        ...
        if current_ts:
            try:
                is_stale_or_no_ts_current = (
                    datetime.now() - datetime.fromtimestamp(int(current_ts))
                ).total_seconds() > 5
            except:
                pass
        ...
        if is_stale_or_no_ts_current:
            logger.debug(
                "Status data for 'status' command is potentially stale (>5s old) or missing timestamp, triggering update..."
            )
            try:
    ...
    async def handle_raw_status_command(self, args: List[str]):
        if self.active_mower_device and self.active_mower_device.mower:
            ...
            if is_stale_or_no_ts_current:
                logger.debug(
                    "Raw status data is potentially stale (>5s old) or missing timestamp, triggering update..."
                )
                try:
            ...
            try:
                ...
                def custom_serializer(obj):
                    if isinstance(obj, (datetime, asyncio.Future, timedelta)):
                        return str(obj)
                ...
                self._last_printed_sys_time_stamp = (
                    self.active_mower_device.mower.report_data.dev.sys_time_stamp
                    if self.active_mower_device.mower.report_data and self.active_mower_device.mower.report_data.dev
                    else None
                )
    ...
    async def handle_maps_plans_command(self, args: List[str]):
        if not (
            self.active_mower_device
            and self.active_mower_device.mower
            and hasattr(self.active_mower_device.mower, "map")
            and self.active_mower_device.mower.map
        ):
        ...
        if map_data.plan and isinstance(map_data.plan, dict) and map_data.plan:
            ...
            print("\n" + table_header_plans + "\n" + table_divider_plans)
            try:
                sorted_plans = sorted(collected_plans, key=lambda p: getattr(p, "remained_seconds", float("inf")))
            except TypeError:
                sorted_plans = sorted(collected_plans, key=lambda p: str(getattr(p, "plan_id", "")))
            if not sorted_plans:
                print("  No plans to display.".ljust(len(table_header_plans)))
            for plan_obj in sorted_plans:
                ...
                knife_height_str = str(getattr(plan_obj, "knife_height", "N/A"))
                speed_val = getattr(plan_obj, "speed", None)
                speed_str = f"{speed_val:.1f}" if isinstance(speed_val, (float, int)) else "N/A"
                days_list = [
                    WEEKDAY_MAP.get(d, f"Day{d}") for d in getattr(plan_obj, "weeks", []) if isinstance(d, int)
                ]
        ...
        if "table_divider_plans" in locals():
            print(table_divider_plans)
        ...
        if map_data.area_name and isinstance(map_data.area_name, list) and map_data.area_name:
            ...
            if not sorted_areas:
                print("  No areas to display.".ljust(len(table_header_areas)))
    ...
    async def _display_mow_defaults_and_usage(self):
        ...
        logger.info(
            f"Defaults are loaded from '{self.config.mow_defaults_file}' if it exists, then overridden by CLI args for a specific job."
        )
            ...
            try:
                if is_enum_setting:
                    ...
                    for member_name_iter in enum_class.__members__:
                        if (
                            member_name_iter.lower() == value_str_cli.lower()
                            or member_name_iter.lower() == value_str_cli.replace("_", "").lower()
                            or member_name_iter.replace("_", "").lower() == value_str_cli.lower()
                        ):
                    ...
                    if matched_member_name:
                        ...
                        logger.error(
                            f"Invalid value '{value_str_cli}' for enum setting '{key_cli_norm}'. Valid names: {list(enum_class.__members__.keys())}"
                        )
                        valid_changes = False
                ...
                logger.error(
                    f"Invalid value format for '{key_cli_norm}': '{value_str_cli}'. Expected type: {expected_type_name}."
                )
                valid_changes = False
    ...
    async def handle_mow_areas_command(self, args: List[str]):
        ...
        try:
            ...
            if not area_hashes:
                raise ValueError("No area hashes provided.")
        except (ValueError, IndexError):
            logger.error("Invalid area hashes for mowing job. Must be comma-separated integers.")
            return
        ...
        for key_default, value_from_defaults_dict in self.mow_defaults.items():
            ...
            try:
                if key_norm in Constants.ENUM_SETTINGS_MAP:
                    ...
                    if enum_value_int is not None:
                        setattr(op_settings, key_norm, enum_value_int)
                    ...
                    if field_type == bool:
                        setattr(op_settings, key_norm, str(value_from_defaults_dict).lower() in ["true", "1", "yes"])
                    elif field_type is int:
                        setattr(op_settings, key_norm, int(value_from_defaults_dict))
                    elif field_type is float:
                        setattr(op_settings, key_norm, float(value_from_defaults_dict))
                    else:
                        setattr(
                            op_settings, key_norm, str(value_from_defaults_dict)
                        )  # Fallback to string if type is unknown
            except Exception as e_set_default:
                logger.warning(
                    f"Could not apply default for {key_norm}={value_from_defaults_dict} to OperationSettings: {e_set_default}"
                )
        ...
        for setting_str in args[1:]:  # CLI overrides
            if "=" not in setting_str:
                logger.warning(f"Skipping invalid setting format: {setting_str}.")
                continue
            key, value_str_cli = setting_str.split("=", 1)
            key_cli_norm = key.lower().strip()
            value_str_cli = value_str_cli.strip()
            try:
                if key_cli_norm in Constants.ENUM_SETTINGS_MAP:
                    ...
                    if enum_value_int is not None:
                        setattr(op_settings, key_cli_norm, enum_value_int)
                elif hasattr(op_settings, key_cli_norm):
                    field_type = type(getattr(op_settings, key_cli_norm, None))
                    if field_type == bool:
                        setattr(op_settings, key_cli_norm, value_str_cli.lower() in ["true", "1", "yes"])
                    elif field_type is int:
                        setattr(op_settings, key_cli_norm, int(value_str_cli))
                    elif field_type is float:
                        setattr(op_settings, key_cli_norm, float(value_str_cli))
                    else:
                        setattr(op_settings, key_cli_norm, value_str_cli)
                else:
                    logger.warning(f"Unknown setting: {key_cli_norm}. Ignored for this job.")
            except ValueError as ve:
                logger.warning(f"Invalid value type for {key_cli_norm}: {value_str_cli}. Error: {ve}. Ignored.")
            except Exception as e_parse:
                logger.warning(f"Generic error parsing setting {key_cli_norm}={value_str_cli}: {e_parse}. Ignored.")
        ...
        try:
            ...
            if not path_order_str or not path_order_str.strip():
                logger.error(f"Generated path_order string is empty! Review op_settings.")
                return
            ...
            gen_route_info = GenerateRouteInformation(
                ...
                rain_tactics=op_settings.rain_tactics,
                speed=op_settings.speed,
                ultra_wave=op_settings.ultra_wave,
                toward=op_settings.toward,
                toward_included_angle=op_settings.toward_included_angle,
                toward_mode=op_settings.toward_mode,
                blade_height=op_settings.blade_height,
                channel_mode=op_settings.channel_mode,
                channel_width=op_settings.channel_width,
                job_mode=op_settings.job_mode,
                edge_mode=op_settings.mowing_laps,
                path_order=path_order_str,
                obstacle_laps=op_settings.obstacle_laps,
            )
            if DeviceType.is_luba1(mower_device_name):
                logger.info(
                    "Luba 1 device detected. Adjusting toward_mode and toward_included_angle to 0 for GenerateRouteInformation."
                )
                gen_route_info.toward_mode = 0
                gen_route_info.toward_included_angle = 0
            logger.debug("Full GenerateRouteInformation object:")
            pprint(dataclasses.asdict(gen_route_info))
            await self._execute_mower_command(
                "generate_route_information",
                params={"generate_route_information": gen_route_info},
                user_friendly_name="mow_generate_route",
            )
            logger.info("'generate_route_information' sent. Waiting briefly...")
            ...
            logger.error(f"Error during mow_areas execution: {e}", exc_info=isinstance(e, ValueError))
        except Exception as e_mow:
    ...
    async def handle_start_command(self, args: List[str]):
        ...
        if not (
            ...
            and hasattr(self.active_mower_device.mower, "map")
            and self.active_mower_device.mower.map
            and hasattr(self.active_mower_device.mower.map, "plan")
            and self.active_mower_device.mower.map.plan
        ):
            ...
            if plan_ref.isdigit():
                ...
                for p_id_key, p_obj in self.active_mower_device.mower.map.plan.items():
                    if str(getattr(p_obj, "plan_id", p_id_key)) == plan_ref:
                        ...
                        logger.info(
                            f"Matched plan by ID: '{getattr(p_obj, 'task_name', 'N/A')}' with ID: {plan_id_to_start}"
                        )
                        found_by_id = True
                        break
                if not found_by_id:
                    logger.warning(
                        f"Plan ID {plan_ref} not found among actual plan IDs in local data. Will attempt to start with '{plan_ref}' as ID if it's purely numeric."
                    )
                    if not plan_ref.isdigit():
                        plan_id_to_start = None  # Ensure it's still None if not a digit after warning
            if not plan_id_to_start:  # Try by name if not found by ID or if original input wasn't a digit
                ...
                if found_plan_obj:
                    ...
                    if not plan_id_to_start:
                        ...
                        return
                    logger.info(
                        f"Found plan by name: '{getattr(found_plan_obj, 'task_name', plan_ref)}' with ID: {plan_id_to_start}"
                    )
                else:
        ...
        try:
            await self._execute_mower_command(
                "single_schedule",
                params={"plan_id": str(plan_id_to_start)},
                user_friendly_name=f"start_plan_{plan_id_to_start}",
            )
    ...
    async def _dispatch_simple_command(
        self,
        user_command_name: str,
        pymammotion_cmd: str,
        params: Optional[Dict] = None,
        request_continuous_updates: bool = False,
    ):
        ...
        try:
            ...
            if request_continuous_updates:
                ...
                if self.active_mower_device:
                    await self._execute_mower_command(
                        "get_report_cfg", user_friendly_name=f"post_{user_command_name}_update"
                    )
            ...
            logger.error(
                f"Unexpected error related to '{user_command_name}' command: {e_simple_unexpected}", exc_info=True
            )
    ...
    async def handle_raw_command(self, args: List[str]):
        ...
        if not param_args:
            ...
            try:
                ...
                if not isinstance(params, dict):
                    logger.error("JSON params must be a valid JSON object.")
                    return
            ...
            for p_arg in param_args:
                ...
                try:  # Attempt to convert to int/float if possible, else bool, else string
                    ...
                    if value_str.lower() == "true":
                        ...
                        params[key] = value_str  # Fallback to string
        ...
        await self._dispatch_simple_command(
            f"raw command {cmd_name}", cmd_name, params, request_continuous_updates=False
        )
    ...
    async def handle_help_command(self, args: List[str]):
        ...
        print(
            f"  sync_maps                     - Attempt to refresh map/plan data (cache in {self.config.map_cache_file})."
        )
        print(f"  mow_areas <show|set <opts..._ HASHES [opts...]")
        print(
            f"                                - 'mow_areas show': Show current defaults from {self.config.mow_defaults_file} & usage."
        )
        print(
            f"                                - 'mow_areas set param=val ...': Set and save defaults to {self.config.mow_defaults_file}."
        )
        print(
            f"                                - 'mow_areas HASHES [opts...]': Start mow job with specified areas & overrides."
        )
        ...
        print(
            '                                  Example: rawcmd request_iot_sys rpt_act=1 rpt_info_type="[1,2,3]" count=0 '
        )
    ...
    async def handle_connect_command(self, args: List[str]):
        ...
        if await self.connect_and_initialize_mower():
            ...
            if self._periodic_status_task and (
                self._periodic_status_task.done() or self._periodic_status_task.cancelled()
            ):
...
def handle_async_exception(loop, context):
    ...
    if isinstance(exception, json.JSONDecodeError):
        ...
        if any(sub in coro_repr for sub in problematic_coroutines_substrings) or (
            future
            and future._source_traceback
            and any("pymammotion" in frame.filename.replace("\\", "/") for frame in future._source_traceback)
        ):
...
async def main_application():
    ...
    logging.basicConfig(
        level=log_level_val, format="%(asctime)s - %(name)s - %(levelname)s - %(message)s", stream=sys.stdout
    )
