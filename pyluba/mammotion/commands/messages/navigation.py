...
class MessageNavigation(AbstractMessage, ABC):
    ...
    def allpowerfull_rw_adapter_x3(self, id: int, context: int, rw: int) -> bytes:
        build = MctlNav(
            nav_sys_param_cmd=NavSysParamMsg(
    ...
    def single_schedule(self, plan_id: str) -> bytes:
        return self.send_order_msg_nav(MctlNav(
            plan_task_execute=NavPlanTaskExecute(
                sub_cmd=1,
                id=plan_id
            )
        ))
    ...
    def set_plan_unable_time(self, sub_cmd: int, device_id: str, unable_end_time: str, unable_start_time: str) -> bytes:
        ...
        return self.send_order_msg_nav(
            MctlNav(todev_unable_time_set=build))
    ...
    def get_all_boundary_hash_list(self, sub_cmd: int):
        build = MctlNav(
            todev_gethash=NavGetHashList(pver=1, sub_cmd=sub_cmd)
        )
    ...
    def get_regional_data(self, regional_data_bean: RegionData):
        build = MctlNav(
            todev_get_commondata=NavGetCommData(
                ...
                data_hash=regional_data_bean.hash, total_frame=regional_data_bean.total_frame,
    ...
    def indoor_simulation(self, flag: int):
        build = MctlNav(
            simulation_cmd=SimulationCmdData(sub_cmd=flag)
        )
