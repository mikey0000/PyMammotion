...
#: Sent to the OSM tile servers, whose usage policy requires a User-Agent that
#: identifies the application.  Defaults to this library rather than naming a
#: particular host — ``pymammotion`` does not know what is embedding it — and hosts
#: that want to identify themselves should set :func:`set_osm_user_agent` at startup.
try:
    _VERSION = version("pymammotion")
except PackageNotFoundError:  # running from a source tree that was never installed
    _VERSION = "unknown"

OSM_USER_AGENT = f"pymammotion/{_VERSION} (+https://github.com/mikey0000/PyMammotion)"


def set_osm_user_agent(user_agent: str) -> None:
    """Identify the embedding application to the OSM tile servers.

    OSM asks for a User-Agent naming the app and a contact route; an app that
    renders a lot of tiles should call this once at startup rather than shipping
    tiles under the library's generic identity.
    """
    global OSM_USER_AGENT  # noqa: PLW0603 — module-level config, set once at startup
